...
public class AddMoneyTransferFlow : UserFlow
{
    ...
    public override async Task Complete(long sessionId, IMediator mediator, CancellationToken ct)
    {
        await mediator.Send(new SaveMoneyTransferCommand { SessionId = sessionId, MoneyTransfer = ToEntity() }, ct);
    }
    ...
    internal override ExtraData GetExtraData()
    {
        return new ExtraData()
        {
            ...
            ShowAllCategories = Draft.ShowAllCategories,
            ConfirmationText = CurrentStep == FlowStep.Confirm
                ? string.Join(Environment.NewLine, ToEntity().ToString(), "", "Would you like to save it?")
                : null
        };
    }
