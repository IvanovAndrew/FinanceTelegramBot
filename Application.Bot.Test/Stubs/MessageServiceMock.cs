...
public class MessageServiceMock : IMessageService
{
    ...
    private readonly List<OutcomingMessageStub> _sentMessages = new();
    public IReadOnlyList<OutcomingMessageStub> SentMessages => _sentMessages.AsReadOnly();
    ...
    {
        var message = new OutcomingMessageStub()
        {
            Id = _messageId++,
            ChatId = chatId,
            Options = options,
    ...
    public Task RemoveMessageButtonsAsync(long chatId, int messageId, CancellationToken cancellationToken = default)
    {
        var sentMessage = SentMessages.FirstOrDefault(m => m.ChatId == chatId && m.Id == messageId);

        if (sentMessage != null)
        {
            sentMessage.Options = null;
        }
    ...
    private OutcomingMessageStub? FindSentMessageById(long chatId, int messageId)
    {
