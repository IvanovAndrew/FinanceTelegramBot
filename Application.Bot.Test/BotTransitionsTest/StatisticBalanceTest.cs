...
public class StatisticBalanceTest 
{
    ...
    public async Task Balance_Since_Previous_Month()
    {
        ...
        // Assert
        var summaryMessage = scenario.MessageService.SentMessages.Last(t => t.Text != null);
        
        Assert.NotNull(summaryMessage);
        Assert.Contains("From June 2023", summaryMessage.Text);
        Assert.Contains("Income", summaryMessage.Text);
        Assert.Contains("Outcome", summaryMessage.Text);
        Assert.Contains("Total balance", summaryMessage.Text);
        
        Assert.Contains("Safe to spend", summaryMessage.Text);
    }
    ...
    public async Task Balance_From_Custom_Month()
    {
        ...
        // Assert
        var summaryMessage = scenario.MessageService.SentMessages.Last(t => !string.IsNullOrEmpty(t.Text));
        
        Assert.NotNull(summaryMessage);
        Assert.Contains("Summary", summaryMessage.Text);
        
        Assert.Contains("From January 2023", summaryMessage.Text);
        Assert.Contains("Income", summaryMessage.Text);
        Assert.Contains("Outcome", summaryMessage.Text);
        Assert.Contains("Total balance", summaryMessage?.Text);
        
        Assert.Contains("Total", summaryMessage.Text);
    }
    ...
    public async Task StatisticFromJanuary_Messages()
    {
        ...
        // Assert
        Assert.NotNull(scenario.MessageService.SentMessages.FirstOrDefault(x => (x.Text?? string.Empty).Contains("Safe to spend")));
    }
